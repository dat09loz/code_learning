...
const Form = ({ type, post, setPost, submitting, handleSubmit }) => {
  return (
    <section className="w-full max-x-full fiex-start flex-col">
      ...
      <form className="mt-10 w-full max-w-2xl flex flex-col gap-7 glassmorphism" onSubmit={handleSubmit}>
        ...
        <label>
          ...
          <input className='form_input' value={post.tag} onChange={(e) => setPost({...post, prompt: e.target.value})} placeholder="#tag" required/>
        </label>

        {/*Buttons*/}
        <div className="flex-end mx-3 mb-5 gap-4">
          <Link href='/' className="text-grey-700 text-sm">
            Cancel
          </Link>
